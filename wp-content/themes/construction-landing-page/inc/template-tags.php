...
function construction_landing_page_posted_on() {
    ...
    if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list && construction_landing_page_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( '%1$s', 'construction-landing-page' ) . '</span>', $categories_list ); // WPCS: XSS OK.
        }
        
        /* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'construction-landing-page' ) );
		if ( $tags_list ) {
			printf( '<span class="tag-links">' . esc_html__( '%1$s', 'construction-landing-page' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
    }
