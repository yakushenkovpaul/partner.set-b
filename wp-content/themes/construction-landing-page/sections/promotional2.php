<?php
/**
 * Promotional Section2
 *
 * @package Construction_Landing_Page2
 */

$section_title   = get_theme_mod( 'construction_landing_page_promotional2_section_page' );
$button_label    = get_theme_mod( 'construction_landing_page_promotional2_button_label' );
$button_link     = get_theme_mod( 'construction_landing_page_promotional2_button_link' );
...
if( $section_title || $button_label || $button_link ){ 
    ...
    if( $section_title && $promotional_query->have_posts() ){ 
        while( $promotional_query->have_posts() ){ $promotional_query->the_post();
            ...
            <section class="promotional-block" <?php echo $style; ?>>
        <?php }
        wp_reset_postdata();
    }else{?>
